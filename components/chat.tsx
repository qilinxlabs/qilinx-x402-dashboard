...
import { X402ServicePanel } from "./chat/x402-service-panel";
import { X402ErrorBoundary } from "./chat/x402-error-boundary";
...
export function Chat({
  ...
  const { mode: signingMode } = useSigningMode();
  const signingModeRef = useRef(signingMode);

  useEffect(() => {
    signingModeRef.current = signingMode;
  }, [signingMode]);
      ...
      prepareSendMessagesRequest(request) {
        ...
        return {
          body: {
            ...
            x402SigningMode: signingModeRef.current,
            ...request.body,
          },
        };
      },
  ...
  // Handle X402 service execution from the panel
  const handleExecuteX402Service = (service: X402Service) => {
    // Create a natural language instruction for the LLM
    const instruction = `Execute the ${service.id} service`;
    ...
    <>
      <div className="overscroll-behavior-contain flex h-dvh min-w-0 touch-pan-y flex-col bg-background">
        ...
        <div className="sticky bottom-0 z-1 mx-auto flex w-full max-w-4xl flex-col gap-0 border-t-0 bg-background">
          ...
          {!isReadonly && (
            <X402ErrorBoundary>
              <X402ServicePanel onExecuteService={handleExecuteX402Service} />
            </X402ErrorBoundary>
          )}
