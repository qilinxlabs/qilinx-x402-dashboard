...
import { Loader2, Wallet, ExternalLink, ArrowRightLeft, Send, Users, Info, Plus, Trash2 } from "lucide-react";
...
interface Split {
  recipient: string;
  bips: number; // basis points (100 = 1%)
}

export function X402SplitPaymentTemplate({ config, contract }: TemplateProps) {
  ...
  const handleConnect = async () => {
    ...
    setError(null);
    try {
      // First, connect to get accounts
      const addr = await connectWallet();
      setWalletAddress(addr);
      
      // Then check network after connection
      const currentNetwork = await getCurrentNetwork();
      if (currentNetwork !== contract.network) {
        const switched = await switchNetwork(contract.network);
        if (!switched) {
          setWrongNetwork(true);
          // Don't throw error - let user use the "Switch Network" button
          return;
        }
      }
  ...
  const addSplit = () => {
    ...
    setSplits([...splits, { recipient: "", bips: 0 }]);
  };
  ...
  const updateSplit = (index: number, field: "recipient" | "bips", value: string | number) => {
    const newSplits = [...splits];
    if (field === "recipient") {
      newSplits[index].recipient = value as string;
    } else {
      newSplits[index].bips = Math.min(10000, Math.max(0, Number(value)));
    }
    setSplits(newSplits);
  };

  const totalBips = splits.reduce((sum, s) => sum + s.bips, 0);
  const isValidSplits = totalBips <= 10000 && splits.every(s => s.recipient && ethers.isAddress(s.recipient));

  const handlePayAndSplit = async () => {
    if (!walletAddress || !isValidSplits) {
      setError("Please configure valid split recipients");
      return;
    }
    ...
    try {
      ...
      // Encode hookData for TransferHook: Split[] array
      ...
      // payTo is the first recipient (receives any remainder)
      const payTo = splits[0].recipient;
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-blue-800 dark:text-blue-200">
                <p className="font-medium mb-1">x402 Split Payment</p>
                <p className="text-blue-700 dark:text-blue-300">
                  Pay {paymentAmount} USDC and split it between multiple recipients in a single atomic transaction.
                  Configure the split percentages below.
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="mb-6">
          <CardContent className="p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant={totalBips === 10000 ? "default" : totalBips > 10000 ? "destructive" : "secondary"}>
                {(totalBips / 100).toFixed(1)}% allocated
              </Badge>
            </div>
                ...
                <div key={index} className="flex gap-2 items-center">
                  ...
                  <div className="flex items-center gap-1 w-24">
                    <Input
                      ...
                      min="0"
                      max="10000"
                      value={split.bips / 100}
                      onChange={(e) => updateSplit(index, "bips", parseFloat(e.target.value) * 100)}
            ...
            {totalBips < 10000 && totalBips > 0 && (
              <p className="text-sm text-muted-foreground">
                Remaining {((10000 - totalBips) / 100).toFixed(1)}% will go to the first recipient
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="max-w-md mx-auto">
          <CardContent className="p-8 space-y-6">
            <div className="text-center">
              <Send className="h-16 w-16 mx-auto mb-4" style={{ color: theme.primaryColor }} />
              <h2 className="text-2xl font-bold mb-2">Pay & Split</h2>
              <p className="text-muted-foreground">
                Pay {paymentAmount} USDC split between {splits.length} recipients
              </p>
            </div>
            ...
            {!walletAddress ? (
              ...
              <Button 
                onClick={handlePayAndSplit} 
                disabled={loading === "pay" || !isValidSplits || totalBips === 0} 
              ...
              >
                {loading === "pay" ? (
                  ...
                  <><Send className="mr-2 h-5 w-5" /> {sections?.paymentButton?.title || "Pay & Split"}</>
                )}
              </Button>
            )}

            {!isValidSplits && walletAddress && !wrongNetwork && (
              <p className="text-sm text-amber-600 text-center">
                ⚠️ Please enter valid recipient addresses
              </p>
            )}
          </CardContent>
        </Card>
            ...
            <CardContent className="p-4">
              {txHash && (
                <div className="flex items-center justify-between text-green-600">
                  <span className="font-medium">Payment Split Successfully!</span>
                  <a
