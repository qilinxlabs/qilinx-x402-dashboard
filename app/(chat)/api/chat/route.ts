...
import { x402Tools, x402ToolNames } from "@/lib/ai/tools/x402-tools";
...
export async function POST(request: Request) {
  ...
  try {
    const { id, message, messages, selectedChatModel, selectedVisibilityType, x402SigningMode } =
      requestBody;
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer: dataStream }) => {
        ...
        // Combine MCP tools with X402 tools
        const allTools = {
          ...mcpContext.tools,
          ...x402Tools,
        };

        // Build active tools list from MCP tools and X402 tools
        const allActiveTools = isReasoningModel ? [] : [...mcpToolNames, ...x402ToolNames];

        const result = streamText({
          model: getLanguageModel(selectedChatModel),
          system: systemPrompt({ selectedChatModel, requestHints, x402SigningMode }),
